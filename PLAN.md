...
| 10 | Cook mode + polish | Week 16 | ✅ **Done (2026-04-26)** | Cook mode at `/cook/[recipeId]` ships with screen-on (`expo-keep-awake`), step counter, large step text, ingredients drawer (collapsed by default), Previous/Next navigation, "Finished! 🎉" on last step, confirm-before-exit on partial cook. Entry: "Start cooking →" CTA in Clean view (recipe detail) when steps exist. Palette applied throughout via `useThemeStore`. |
| 10.5 | Editor UX polish | Week 16 | ✅ **Done (5/5, 2026-04-26)** | Help overlay (`HelpSheet.tsx`, 2026-04-22), Edit recipe from Clean view (`/recipe/edit/[id]`, 2026-04-25), 10.5c canvas pinch-zoom + pan + double-tap reset + zoom pill (`CanvasPageZoom.tsx`, 2026-04-26), Apple Pencil pressure (already wired via `e.pressure ?? 0.5` + `simulatePressure: false` — cleaned up the cast 2026-04-26), custom colour picker (16 swatches in two rows + `ColorPickerModal.tsx` HSL grid for any colour, 2026-04-26). |
...
| 10.7a | Magic-link sign-in + password reset | — | ✅ Done (2026-04-26) | `sendMagicLink()`, `requestPasswordReset()`, `updatePassword()` in `src/api/auth.ts`. Login screen has "Send me a magic link" + "Forgot password?" actions. Deep-link handler `src/hooks/useAuthDeepLink.ts` parses `spoonsketch://auth/callback` and `spoonsketch://auth/reset` URLs, calls `setSession`, and routes to `/(auth)/reset-password` for the reset flow. **Pending Angy**: add the redirect URLs to Supabase Dashboard → Auth → URL Configuration. |
...
| 10.7d | Telemetry SDK scaffolds (PostHog + Sentry) | — | ✅ Done (2026-04-26) | `src/lib/analytics.ts` PostHog wrapper with typed `AnalyticsEventMap` (per CLAUDE rule #6) — no-op until `EXPO_PUBLIC_POSTHOG_KEY` set. `src/lib/sentry.ts` ErrorBoundary now reports to Sentry — no-op until `EXPO_PUBLIC_SENTRY_DSN` set. PostHog gated by tracking-consent banner; Sentry not gated (legitimate interest per existing design). |
