...
1. ✅ **Remaining 5 templates landed 2026-04-22.** Photo Hero / Minimal / Two Column / Journal / Recipe Card render functions ported into `renderRecipePage.ts`. Each has its own template-specific CSS (`.t-photo-hero`, `.t-minimal`, `.t-two-column`, `.t-journal`, `.t-recipe-card`) and shared helpers (`pillsInline`, `ingRowsDotted`/`Plain`/`Journal`, `stepRowsBadged`/`Numeric`). Render dispatch in `renderTemplate` now picks the right function per template.
2. ✅ **Stickers + drawing strokes landed 2026-04-22.** `renderRecipePage` now emits:
   - A full-page SVG `<svg class="drawing">` with one `<g>` per drawing layer (CSS `mix-blend-mode` mirrors the RN blend modes; `opacity` honors the layer's opacity; invisible layers skipped). Each stroke runs through `perfect-freehand` → quadratic-Bezier path `d` attribute, same algorithm used in `src/components/canvas/DrawingStroke.tsx`. Eraser strokes are skipped in v1 print (proper handling needs SVG masks; tracked as known scope).
   - An `<img class="sticker">` per sticker element, absolute-positioned from the RecipePage fractional `cx/cy`, `width/height` 64px, `transform: rotate(…) scale(…)`, z-index stacking from the sticker's own zIndex.
   - `renderRecipePage` takes a new `stickerSrc?: (kind) => string | null` option so the renderer stays pure. `exportRecipePdf` resolves each sticker kind to its bundled PNG's local `file://` URI via `expo-asset` before invoking `renderRecipePage` (pre-resolved so the resolver can stay synchronous).
   - Added dep: `expo-asset` (bundled in Expo Go for SDK 54).
