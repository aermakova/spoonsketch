...
import { MANIFEST, STYLE_PREFIX, type StickerManifestEntry } from './stickers-manifest';
...
async function main() {
  ...
  const entry: StickerManifestEntry | undefined = MANIFEST.find(
  ...
  if (!entry) {
    ...
    for (const m of MANIFEST) console.error(`  ${m.pack}/${m.id}`);
    process.exit(1);
  }
